...
const AssignToLesson = () => {
    ...
    const handleAssignToLesson = async () => {
        try {
            ...
            if (!response.ok) {
                ...
                const errorMessage =
                    errorBody?.error || `Error ${response.status}: Unable to join the lesson. Please try again later.`;
                throw new Error(errorMessage); // Throw error with detailed message
            }

            setDoesChanged(true); // Notify that lessons have changed
            Alert.alert("Success", "You have successfully joined the lesson", [
                { text: "OK", onPress: () => router.back() },
            ]);
        } catch (error) {
            Alert.alert(
                "Error",
                error instanceof Error ? error.message : "Unable to join the lesson. Please try again later."
            );
        } finally {
    ...
    return (
        <View style={styles.container}>
            <Text style={styles.title}>Are you sure you want to assign to the lesson?</Text>
            <Text style={styles.warningText}>
                By joining this lesson, you confirm your participation.
            </Text>
            <Pressable
                style={styles.assignButton}
                onPress={handleAssignToLesson}
...
const styles = StyleSheet.create({
    ...
    assignButton: {
        ...
        height: 70,
        borderRadius: 35,
        backgroundColor: "#32CD32", // Green color for assign
        justifyContent: "center",
        alignItems: "center",
        shadowColor: "#000",
